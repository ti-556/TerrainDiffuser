...
class TrainCfg:
    ...
    sat_dir       : str   = "data/SAT"
    h5_path       : str   = "data/text.h5"
    val_size      : int   = 32          # first N samples → validation
    num_workers     : int = 4     
    prefetch_factor : int = 2            

    # ---- VGG feature extractor ----
    feature_channels : dict[int,int] = field(default_factory=lambda:{16:128})
...
class Trainer:
    ...
    def __init__(self, cfg: TrainCfg):
        ...
        if prec == "fp32":
            ...
            self._amp = lambda: autocast(device_type="cuda", dtype=torch.bfloat16)
            self._amp_dtype = torch.bfloat16
        else:                              # default fp16
            self._amp = lambda: autocast(device_type="cuda", dtype=torch.float16)
    ...
    def train(self):
        for epoch in range(self.cfg.epochs):
            self.model.train()
            for dem, sat, txt in self.tr_loader:
    ...
    def _flow_euler(x_t, vf_hat, dt):
        return (x_t + dt*vf_hat).clamp_(-1,1)
